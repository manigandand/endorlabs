...
ARGS="--out-format=${OUT_FORMAT} --issues-exit-code=2 --path-prefix=kpi-store --config=.golangci.yaml"

CMD_CHECK_ALL="${LINTER} run ${ARGS} ${SCAN_PATH}"
echo "Running ${CMD_CHECK_ALL}";
CHECK_ALL_LINT_LOG=$CMD_CHECK_ALL
